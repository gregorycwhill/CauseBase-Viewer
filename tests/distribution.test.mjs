...
test("static build provides indexable direct routes and machine release discovery", () => {
  const output=mkdtempSync(join(tmpdir(),"charitygraph-viewer-distribution-"));
  const localRelease=resolve(root,"..","charitygraph-data","releases","v0.5.0-2026-08-15");
  const ciRelease=resolve(root,"charitygraph-data","releases","v0.5.0-2026-08-15");
  const release=existsSync(localRelease) ? localRelease : ciRelease;
  execFileSync(process.execPath,[join(root,"scripts","prepare-pages.mjs")],{cwd:root,env:{...process.env,CHARITYGRAPH_OUTPUT_DIR:output,CHARITYGRAPH_DATA_DIR:release},stdio:"pipe"});
  ...
  assert.match(page,new RegExp(`<link rel="canonical" href="https://gregorycwhill.github.io/charitygraph-viewer/charity/${card.causebase_id}/">`));
  assert.match(page,/Card JSON/); assert.match(page,/Source-native records/);
  assert.doesNotMatch(page,/CauseBase/);
